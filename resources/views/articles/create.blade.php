...
@section('content')
    ...
    <form action="{{ route('articles.store') }}" method="POST" class="article-form">
        ...
        <textarea id="short_desc" name="short_desc" rows="3" required>{{ old('short_desc') }}</textarea>
        ...
        <input id="preview_image" type="text" name="preview_image" value="{{ old('preview_image') }}">

        <label for="full_image">Имя full-файла (например full.jpeg)</label>
        <input id="full_image" type="text" name="full_image" value="{{ old('full_image') }}">
